...
const Slider = () => {
  ...
  const PreButton = () => {
    setCurrent(
      (preData) => (preData - 1 + sliders.length) % sliders.length
    );
  };
    ...
    <main className="py-10">
      <div className=" space-y-8 ">
        ...
        <div className="max-w-[920px] min-w-[200px] mx-auto space-y-1">
          ...
          <h4 className="text-gray-400 font-semibold text-center">{sliders[current].company }</h4>
        </div>
        <div className="flex justify-between px-10 md:-translate-y-32 min-w-[400px] max-w-[1300px] mx-auto">
          <button onClick={PreButton} className="text-white bg-[#96ACAF33] px-3 py-2 rounded-lg active:scale-95 transition-all hover:bg-[#96acaf1e]"><ArrowLeft className="text-[#96ACAF]"/></button>
          <button onClick={Nextbutton} className="text-white bg-[#96ACAF33] px-3 py-2 rounded-lg active:scale-95 transition-all hover:bg-[#96acaf1e]"><ArrowRight className="text-primary"/></button>
        </div>
      </div>
    </main>
  );
